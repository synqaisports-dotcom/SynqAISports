    ...
    <div className="space-y-8 animate-in fade-in duration-1000">
      ...
      <div className="flex flex-col gap-2 border-b border-white/5 pb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-primary tracking-[0.5em] uppercase">Global_Command_Center</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card className="glass-panel lg:col-span-2 overflow-hidden relative group">
          <div className="absolute top-0 right-0 p-1 bg-primary/20 text-[8px] font-black px-2 uppercase tracking-widest text-primary">Live_Network_Traffic</div>
          <CardHeader>
            <CardTitle className="text-sm uppercase tracking-widest font-black flex items-center gap-2">
              <Activity className="h-4 w-4 text-primary" /> Monitoreo de Flujos Globales
            </CardTitle>
          </CardHeader>
          <CardContent className="h-80 flex flex-col items-center justify-center m-6 mt-0 border border-primary/10 bg-black/40 relative overflow-hidden">
             ...
             <div className="relative z-10 text-center space-y-4">
                <span className="text-[10px] font-black text-primary uppercase tracking-[0.5em] animate-pulse">Analizando_Nodos_Tácticos...</span>
        ...
        <Card className="glass-panel">
          ...
          <div className="p-4">
             <Button variant="ghost" className="w-full h-10 rounded-none border border-white/5 text-[9px] font-black uppercase tracking-widest hover:text-primary transition-all" asChild>
  ...
  return (
    <Card className="glass-panel relative overflow-hidden group hover:scale-[1.02] transition-all">
      ...
      <CardHeader className="pb-2">
        ...
        <CardTitle className="text-3xl font-black text-white group-hover:cyan-text-glow transition-all italic tracking-tighter">{value}</CardTitle>
      </CardHeader>
      <CardContent>
...
function LogItem({ type, title, desc }: any) {
  const color = type === 'Success' ? 'text-emerald-400' : type === 'Warning' ? 'text-rose-400' : 'text-primary';
  return (
    <div className="p-5 hover:bg-white/[0.02] transition-colors group cursor-default">
      <div className="flex items-center gap-3 mb-1">
         <div className={`h-1.5 w-1.5 rounded-full animate-pulse ${type === 'Success' ? 'bg-emerald-400' : type === 'Warning' ? 'bg-rose-400' : 'bg-primary'}`} />
