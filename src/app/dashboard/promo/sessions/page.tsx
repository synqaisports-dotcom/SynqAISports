...
/**
 * Mi Agenda Promo - v9.44.0
 * Incluye la exportación de Ficha de Sesión PDF (3 bloques en 1 hoja).
 */
               ...
               {session ? (
                 <Card className="glass-panel border-primary/30 bg-primary/5 rounded-[2.5rem] overflow-hidden relative group print:bg-white print:border-black/10">
                    <CardHeader className="p-8 border-b border-white/5 bg-black/40 print:bg-black/5 print:border-black/10">
                       <CardTitle className="text-xl font-black text-white italic tracking-tighter uppercase leading-none print:text-black">SESIÓN_ACTIVA</CardTitle>
                 ...
                 <div className="h-[400px] border-2 border-dashed border-white/5 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 bg-white/[0.01] group hover:border-primary/20 hover:bg-primary/[0.02] transition-all print:hidden">
...
function SessionPart({ label, status }: { label: string, status: string }) {
  return (
    <div className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-2xl group hover:border-primary/20 transition-all print:bg-white print:border-black/10">
       <div className="flex flex-col">
          <span className="text-[9px] font-black text-white/40 uppercase tracking-widest print:text-black/40">{label}</span>
          <span className="text-[10px] font-black text-white uppercase italic mt-0.5 print:text-black">{status}</span>
       </div>
