...
function PromoBoardContent() {
  ...
  useEffect(() => {
    ...
    if (savedTeam) { setTeamConfig(savedTeam); if (!exerciseId) setFieldType(savedTeam.type || "f11"); }
    if (exerciseId) {
  ...
  const loadTeamFromSandbox = () => {
    ...
    const teamElements: DrawingElement[] = savedTeam.starters.filter((name: string) => name.trim() !== "").map((name: string, i: number) => {
      ...
      return { id: `el-team-${Date.now()}-${i}`, type: 'player', points: [{ x: pos.x - defW/2, y: pos.y - defH/2 }, { x: pos.x + defW/2, y: pos.y + defH/2 }], color: currentColor, rotation: 0, lineStyle: 'solid', number: i + 1, opacity: 1.0 };
    });
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if (selectedIds.length === 1) {
      ...
      if (el) {
        ...
        const pad = 12 * renderScale; const handles = [{ x: bounds.minX - pad, y: bounds.minY - pad }, { x: bounds.centerX, y: bounds.minY - pad }, { x: bounds.maxX + pad, y: bounds.minY - pad }, { x: bounds.minX - pad, y: centerY }, { x: bounds.maxX + pad, y: centerY }, { x: bounds.minX - pad, y: maxY + pad }, { x: bounds.centerX, y: maxY + pad }, { x: bounds.maxX + pad, y: maxY + pad }];
  ...
  return (
    <div className={cn("h-full w-full flex flex-col bg-black overflow-hidden relative", isLegacyDevice && "perf-lite")}>
      {showAds && isHalfField && (
        <>
      ...
      <header className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center gap-4 w-full max-w-4xl">
        <div className="flex items-center gap-3 px-4 py-2 bg-black/60 backdrop-blur-2xl border border-primary/30 rounded-2xl shadow-2xl animate-in slide-in-from-top-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={loadTeamFromSandbox} className="h-8 px-2 border-primary/20 bg-primary/10 text-primary font-black uppercase text-[7px] rounded-lg hover:bg-primary hover:text-black transition-all"><Users className="h-3 w-3 mr-1" /> Cargar Mi Equipo</Button>
