...
export async function loadMicrocycleBundle(
  ...
  const { data: slots } = await supabase
    ...
    .select(
      'id, session_index, slot_type, order_index, title, notes, exercise_id, sheet_json, session_date, synq_exercises(id, title)'
    )
...
export async function loadExerciseLibrary(
  ...
  const { data } = await supabase
    .from('synq_exercises')
    .select('id, title, task_type, objectives, notes, sheet_json')
    .eq('club_id', clubId)
    .order('title');
  return data ?? [];
}
