...
export type PlayerMaterialKind = 'player-own' | 'player-rival' | 'player-neutral';

const PLAYER_COLORS: Record<PlayerMaterialKind, [string, string]> = {
  'player-own': ['#0ea5e9', '#0369a1'],
  'player-rival': ['#f43f5e', '#be123c'],
  'player-neutral': ['#f59e0b', '#b45309'],
};

export function playerImageKey(kind: MaterialKind, label?: string): string {
  if (!isPlayerMaterial(kind)) return kind;
  return `${kind}:${playerLabelFor(kind, label)}`;
}

function renderPlayerMaterial(kind: PlayerMaterialKind, label: string): string {
  const canvas = document.createElement('canvas');
  canvas.width = TEXTURE_SIZE;
  canvas.height = TEXTURE_SIZE;
  const ctx = canvas.getContext('2d');
  if (!ctx) throw new Error('Canvas 2d unavailable');
  const [fill, stroke] = PLAYER_COLORS[kind];
  drawPlayer(ctx, fill, stroke, label);
  return canvas.toDataURL('image/png');
}

export async function getPlayerImage(
  kind: PlayerMaterialKind,
  label?: string
): Promise<HTMLImageElement> {
  if (typeof document === 'undefined') {
    throw new Error('Material assets require browser');
  }
  const resolved = playerLabelFor(kind, label);
  const key = `${CACHE_VERSION}:player:${kind}:${resolved}`;
  const cached = cache.get(key);
  if (cached) return cached;
  const img = await createImage(renderPlayerMaterial(kind, resolved));
  cache.set(key, img);
  return img;
}
