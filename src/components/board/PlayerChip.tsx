...
export function PlayerChip({ number, team, x, y, label, className }: PlayerChipProps) {
  const isLocal = team === "local";
  
  return (
    <div 
      className={cn(
        "absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-1 transition-all duration-700 ease-in-out z-20",
        className
      )}
      style={{ left: `${x}%`, top: `${y}%` }}
    >
