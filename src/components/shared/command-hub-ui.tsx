  ...
  if (!adsAllowed) {
    return (
      <HubPanel>
        <SectionBar
          title={sectionTitle}
          right={<Sparkles className="h-4 w-4 text-cyan-400/40 drop-shadow-[0_0_8px_rgba(34,211,238,0.35)]" />}
        />
        <div className="p-4">
          <div className="min-h-[120px] w-full flex items-center justify-center px-4 text-center text-[9px] font-black uppercase tracking-[0.18em] text-white/35 border border-dashed border-white/10 bg-black/20">
            Publicidad web desactivada en esta ruta (política WEB-ONLY / visualizador). En app nativa: AdMob.
          </div>
        </div>
      </HubPanel>
    );
